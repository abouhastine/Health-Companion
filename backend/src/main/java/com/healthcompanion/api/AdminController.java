...
public class AdminController {
  ...
  public AdminController(PractitionerRepository practitioners, AppointmentSlotRepository slots,
      UserRepository users, AppointmentRepository appointments, MedicalDocumentRepository docs,
      LocalDocumentStorage storage, DocumentIngestionService ingestion) {
    this.practitioners = practitioners; this.slots = slots; this.users = users;
    this.appointments = appointments; this.docs = docs; this.storage = storage; this.ingestion = ingestion;
  }

  @GetMapping("/patients") public List<User> patients() {
  ...
  @GetMapping("/practitioners") public List<Practitioner> practitionerList() { return practitioners.findAll(); }

  @PostMapping("/practitioners") @ResponseStatus(HttpStatus.CREATED)
  public Practitioner create(@RequestBody Practitioner practitioner) { return practitioners.save(practitioner); }
  ...
  @DeleteMapping("/practitioners/{id}") @ResponseStatus(HttpStatus.NO_CONTENT)
  public void delete(@PathVariable Long id) { practitioners.deleteById(id); }
  ...
  @PostMapping("/practitioners/{id}/slots") @ResponseStatus(HttpStatus.CREATED)
  public AppointmentSlot createSlot(@PathVariable Long id, @RequestBody SlotRequest request) {
    var slot = new AppointmentSlot();
    slot.practitioner = practitioners.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    apply(slot, request);
  ...
  public AppointmentSlot updateSlot(@PathVariable Long id, @RequestBody SlotRequest request) {
    var slot = slots.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    apply(slot, request);
    return slots.save(slot);
  }

  @DeleteMapping("/slots/{id}") @ResponseStatus(HttpStatus.NO_CONTENT)
  public void deleteSlot(@PathVariable Long id) { slots.deleteById(id); }

  @PostMapping(value = "/documents", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
  @ResponseStatus(HttpStatus.CREATED)
  public MedicalDocument upload(@RequestParam Long patientId, @RequestParam DocumentType documentType,
      @RequestParam String title, @RequestParam String documentDate,
      @RequestParam(required = false) Long practitionerId, @RequestPart MultipartFile file) throws IOException {
    ...
    document.patient = users.findById(patientId).filter(user -> user.role == Role.PATIENT)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Patient not found"));
    document.practitioner = practitionerId == null ? null : practitioners.findById(practitionerId)
  ...
  private void apply(AppointmentSlot slot, SlotRequest request) {
    if (request.startAt() == null || request.endAt() == null || !request.endAt().isAfter(request.startAt())) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Slot end time must be after its start time");
    }
